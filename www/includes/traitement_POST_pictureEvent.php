...
if(isset($_SESSION['id'])){
	...
	if(isset($_FILES) && is_array($_FILES) && count($_FILES)) {
		...
		$getname = $bdd->prepare("SELECT title FROM event WHERE ID = ?");
		$getname->execute(array(htmlspecialchars($_GET['id'])));
		$infoname = $getname->fetch();
		$filename = clean($infoname['title']);
		...
		$location = "../uploads/".$prefix.$filename.".".$extensionfull;
			/* Upload file */
	      	if(move_uploaded_file($tmp,$location)){
		      	...
		      	/* EDIT DTB */
		      	$uppicture = $bdd->prepare("UPDATE event SET photo = ? WHERE ID = ?");
		      	if($uppicture->execute(array($prefix.$filename.".".$extensionfull, htmlspecialchars($_GET['id'])))){
		         	$json['200'] = $location;
		      	}
		      	else{
		      		$json['400'] = $location;

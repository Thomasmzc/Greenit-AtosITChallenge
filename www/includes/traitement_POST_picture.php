...
if(isset($_SESSION['id'])){
	...
	if(isset($_FILES) && is_array($_FILES) && count($_FILES)) {
		...
		$getname = $bdd->prepare("SELECT fname, lname FROM members WHERE ID = ?");
		$getname->execute(array($_SESSION['id']));
		$infoname = $getname->fetch();
		$filename =  clean($infoname['fname']." ".$infoname['lname']);
		...
		$location = "../uploads/".$prefix.$filename.".".$extensionfull;
			/* Upload file */
	      	if(move_uploaded_file($tmp,$location)){
		      	...
		      	/* EDIT DTB */
		      	$uppicture = $bdd->prepare("UPDATE members SET photo = ? WHERE ID = ?");
		      	if($uppicture->execute(array($prefix.$filename.".".$extensionfull, $_SESSION['id']))){
		         	$json['200'] = $location;
		      	}
		      	else{
		      		$json['400'] = $location;

...
if(isset($_SESSION['id']) && $_SESSION['type'] == "pro"){
	...
	$insert = $bdd->prepare("INSERT INTO event(organization, id_author, title, location, longitude, latitude, date_start, date_end, hour_start, hour_end, description, photo, website, registration, physical, timing, payment, openness, timestart, timeend, duration, typeduration, topic) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	if($insert->execute(array($_SESSION['orga'], $_SESSION['id'], $title, $location, $longitud, $latitud, $date_start, $date_end, $hour_start, $hour_end, $description, $photo, $website, $registration, $physical, $timing, $payment, $openness, $numbertime, $numbertimeend, $duration, $typeduration,$topic))){

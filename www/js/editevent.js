...
$(document).ready(function(){
  setTimeout(function(){ 
    ...
    if($hour_end.length > 0){
      $("#blchourend").show();
      $("#addendH").text('Remove the end hour');
    }
    if($date_end.length > 0){
      $("#blcdateendt").show();
      $("#addendD").text('Remove the end date');
    }
  }, 1000);
});
/* select charac on load */
$(document).ready(function(){
  setTimeout(function(){ 
    // Get data 
    $idevent = $("#submit2").data('id');
    $.ajax({
        url: 'includes/traitement_GET_caracEvent.php',
        type: 'post',
        dataType: "json",
        data: {
          idevent: $idevent
        ...
        success: function(json){
         $.each(json, function(index, value){
              /* right carac */
              $card = $(".eff"+value[0]);
              $card.siblings().data('selec', "0");
              $card.data('selec', "1");
              $color= "#00B34B";
              $('.roundeff').css({
                "background-color": "#DDDDDD",
              });
              $card.children('.roundeff').css({
...
$("#submit").click(function(){
  ...
  if($title.length > 1 && $location.length > 1 && $date_start.length == 10 && $hour_start.length == 5){
    if(d1 <= d2){
            ...
            $("#submit2").click(function(){
              ...
              setTimeout(function(){
                if($physical > 0 && $timing != 0 && $payment != 0 && $openness != 0){
                  ...
                  $.ajax({
                    url: 'includes/traitement_POST_modifEvent.php',
                    type: 'post',
                    dataType: "json",
                    data:{
                      idevent: $idevent,
  ...
  save.addEventListener('click',(e)=>{
    ...
    if (cropper) {
      ...
      canvas.toBlob(function (blob) {
        ...
        $.ajax({
          url: 'includes/traitement_POST_pictureEvent.php?id='+$idevent,
          type: 'post',
          dataType: "json",
          data: formData,
          contentType: false,
